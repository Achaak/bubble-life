...
export const hasActionInWaitingListByName = ({
  name,
}: HasActionInWaitingListByName): boolean => {
  const { waitingList } = getActions();

  return waitingList.filter((item) => item.name === name).length > 0;
};

export const hasActionInWaitingListById = ({
  id,
}: HasActionInWaitingListById): boolean => {
  const { waitingList } = getActions();
...
export const removeActionFromWaitingList = ({
  id,
}: RemoveActionFromWaitingList): void => {
